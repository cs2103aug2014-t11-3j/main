...
public class UserInterface extends JFrame {
	...
	// this method consists of setting the different sections within the frame
	// of ToDoLog
	...
	private void createToDoListTable(Container mainPanel) {
		...
		GridBagConstraints panelParameters; // panelParameters are values for
											// how the top panel will fit into
											// the main frame of ToDoLog
		GridBagConstraints scrollPaneParameters; // scrollPaneParameters are
													// values for how the
													// scrollPane will be placed
													// within the top
													// panel,toDoListHolder
	...
	// this method creates the bottom section of ToDoLog which consists of the
	// command entry
	...
	private void createCommandEntryTextBox(JPanel bottomPanel) {
		...
		commandEntryTextField
				.addActionListener(new CommandEntryTextFieldListener());
	}
	...
	private void createLegend(JPanel bottomPanel) { // this one will be put at
													// borderlayout.east
		...
		JPanel legendMainPanel = new JPanel(new GridBagLayout()); // must find a
																	// better
																	// way of
																	// organising
																	// the
	...
	private void arrangeLegend(JPanel legendMainPanel) {
		...
		legendPanelLayout = new GridBagConstraints(0, 0, 1, 1, 0.1, 0.0,
		...
		legendTextLayout = new GridBagConstraints(1, 0, 5, 1, 0.1, 0.0,
				GridBagConstraints.CENTER, GridBagConstraints.BOTH, insets, 0,
				0);
		...
		legendPanelLayout = new GridBagConstraints(0, 1, 1, 1, 0.1, 0.0,
				GridBagConstraints.CENTER, GridBagConstraints.BOTH, insets, 0,
				0);
		...
		legendTextLayout = new GridBagConstraints(1, 1, 5, 1, 0.1, 0.0,
				GridBagConstraints.CENTER, GridBagConstraints.BOTH, insets, 0,
				0);
		...
		legendPanelLayout = new GridBagConstraints(0, 2, 1, 1, 0.1, 0.0,
				GridBagConstraints.CENTER, GridBagConstraints.BOTH, insets, 0,
				0);
		...
		legendTextLayout = new GridBagConstraints(1, 2, 5, 1, 0.1, 0.0,
				GridBagConstraints.CENTER, GridBagConstraints.BOTH, insets, 0,
				0);
		...
		legendPanelLayout = new GridBagConstraints(0, 3, 1, 1, 0.1, 0.0,
				GridBagConstraints.CENTER, GridBagConstraints.BOTH, insets, 0,
				0);
		...
		legendTextLayout = new GridBagConstraints(1, 3, 5, 1, 0.1, 0.0,
				GridBagConstraints.CENTER, GridBagConstraints.BOTH, insets, 0,
				0);
	...
	private GridBagConstraints setParameters(int panelParameters) {
		...
		if (panelParameters == TODOLIST_PARAMETERS) {
			parameters = new GridBagConstraints(0, 0, 3, 3, 0.1, 0.0,
			...
			parameters = new GridBagConstraints(0, 3, 3, 1, 0.0, 0.3,
			...
			parameters = new GridBagConstraints(0, 0, 3, 1, 0.1, 0.0,
					GridBagConstraints.NORTHWEST, GridBagConstraints.BOTH,
					insets, 0, 0);
			...
			parameters = new GridBagConstraints(0, 1, 2, 1, 0.0, 0.1,
					GridBagConstraints.CENTER, GridBagConstraints.BOTH,
					dynamicHelpTextInsets, 0, 0);
			return parameters;
		}

		else if (panelParameters == LEGEND_PARAMETERS) {
			parameters = new GridBagConstraints(2, 1, 1, 1, 0.0, 0.1,
			...
			parameters = new GridBagConstraints(0, 0, 1, 1, 0.0, 0.0,
					GridBagConstraints.CENTER, GridBagConstraints.BOTH, insets,
					0, 0);
			return parameters;
		}
	...
	// toDoListItems.add(new
	// Task("3","Brilliant meeting with the incredible hulk","02/12/1992","Medium","Remember to bring shotgun"));
	// }
	...
	private void adjustTableColumns(JTable toDoListTable) {
		...
		for (int columnHeaders = 0; columnHeaders < 5; columnHeaders++) {
			tableColumn = toDoListTable.getColumnModel().getColumn(
					columnHeaders);
	...
	private void changeTableColors(JTable toDoListTable) {
		toDoListTable.getColumnModel().getColumn(0)
				.setCellRenderer(new CustomRenderer());
		toDoListTable.getColumnModel().getColumn(1)
				.setCellRenderer(new CustomRenderer());
		toDoListTable.getColumnModel().getColumn(2)
				.setCellRenderer(new CustomRenderer());
		toDoListTable.getColumnModel().getColumn(3)
				.setCellRenderer(new CustomRenderer());
		toDoListTable.getColumnModel().getColumn(4)
				.setCellRenderer(new CustomRenderer());
