...
public class TaskTest {
	...
	public void testParseName() throws Exception {
		...
		assertEquals("output should be group meeting", "group meeting",
				task.getTaskName());
		Task task2 = new Task("\"group meeting");
		assertEquals("output should be invalid input", "Invalid Input!",
				task2.getTaskName());
	...
	public void testParseType() throws Exception {
		Task task = new Task("\"group meeting\"");
		assertEquals("output should be FLOATING", TaskType.FLOATING,
				task.getTaskType());
		Task task2 = new Task("\"group meeting\" from friday to saturday");
		assertEquals("output should be TIMED", TaskType.TIMED,
				task2.getTaskType());
		Task task3 = new Task("\"group meeting\" by friday");
		assertEquals("output should be DEADLINE", TaskType.DEADLINE,
				task3.getTaskType());
		Task task4 = new Task("\"group meeting\" every friday");
		assertEquals("output should be RECURRING", TaskType.RECURRING,
				task4.getTaskType());
		Task task5 = new Task("\"group meeting every friday");
		assertEquals("output should be INVALID", TaskType.INVALID,
				task5.getTaskType());
	}
	...
	public void testParseDay() throws Exception {
		...
		assertEquals("output should be Thursday", "Thursday",
				task2.getStartDay());
		...
		assertEquals("output should be Wednesday", "Wednesday",
				task5.getEndDay());
	}
	...
	public void testTime() throws Exception {
		...
		try {
			Task task4 = new Task(
					"\"group meeting\" from fri @ 11h00 to sat @ 16h50");
		} catch (Exception e) {
			// pass
		}
